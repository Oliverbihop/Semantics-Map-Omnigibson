...
ros_ws = "/home/luan/ros2_behavior/"
...
def generate_map_yaml(w, h):
    ...
    with open(ros_ws + "src/behavior/maps/trav_map.yaml", "w") as f:
...
def main():
    ...
    scene_model = choose_from_options(options=scenes, name="scene model", random_selection=False)
    print(f"Generating traversability map for scene {scene_model}")

    trav_map_erosion = 2

    trav_map = cv2.imread(os.path.join(get_scene_path(scene_model), "layout", "floor_trav_0.png"))
    trav_map = cv2.erode(trav_map, th.ones((trav_map_erosion, trav_map_erosion)).cpu().numpy())

    cv2.imwrite(ros_ws + "src/behavior/maps/trav_map.png",trav_map)
