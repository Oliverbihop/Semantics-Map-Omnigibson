    ...
    while step != max_steps:
        ...
        try:
            ...
            if camera_sensor is not None and hasattr(camera_sensor, "intrinsic_matrix"):
                ...
                fx = fx if fx != 0 else 1e-6
                fy = fy if fy != 0 else 1e-6

                # Fixed: Get intrinsics from the camera sensor, not the viewer camera
                # The viewer camera is different from the robot's camera sensor
                # Use the camera sensor's intrinsic matrix that we already have
        ...
        if R_wc_from_view is not None and t_wc_from_view is not None:
            ...
            # Fixed: Use the camera sensor's rotation matrix instead of accessing viewer_camera
            # The R_wc_from_view already contains the correct rotation matrix
            try:
